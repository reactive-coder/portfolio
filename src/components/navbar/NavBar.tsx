...
const NavBar: Function = () => {
    ...
    return (
        <nav className="font-sans w-full sticky top-0 z-20 before:-z-10 before:bg-black before:opacity-20 before:absolute before:top-0 before:h-full before:w-full text-gray-50">
            <Container>
            ...
            {showSideBar ?
                <div className="h-screen fixed top-0 left-0 bg-gray-700 flex flex-col">
                    <span className="pl-5 my-4 pr-20 text-2xl tracking-wider uppercase">{home.name}</span>
                    {navData.map((item, i) => (
                        <Link key={i.toString()}
